      ...
      <div className="flex w-max animate-astro-scroll">
        {scrollingItems.map((item, i) => (
          <Link
            ...
            href={item.href}
            className="flex shrink-0 flex-col items-center text-center gap-1 mx-3 py-2 px-3 rounded-2xl border border-fuchsia-500/40 bg-fuchsia-500/10 text-xs text-foreground/90 hover:bg-fuchsia-500/15 transition-colors max-w-[34rem]"
            title={item.question}
          >
            <span className="leading-relaxed break-words whitespace-normal">{item.question}</span>
            <span className="inline-flex items-center gap-1.5 opacity-90">
              <span>{item.leadingPercent}</span>
              <span className="opacity-70">-</span>
              <span className="font-semibold text-foreground">{item.leadingSide}</span>
            </span>
          </Link>
        ))}
      </div>
